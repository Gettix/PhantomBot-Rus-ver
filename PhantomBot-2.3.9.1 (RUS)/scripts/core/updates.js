...
(function() {
    ...
    if ($.changed == true && $.changed != null && $.changed != undefined && !$.inidb.exists('updates', 'installedNewBot') && $.inidb.get('updates', 'installedNewBot') != 'true') {
        $.consoleLn('Инициализирую версию PhantomBot ' + $.version + ' для Первого запуска...');
        ...
        $.consoleLn('Отключаем стандартные модули...');
        for (i in modules) {
            $.inidb.set('modules', modules[i], 'false');
        }

        $.consoleLn('Добавляем новые стандартные команды...');
        $.inidb.set('command', 'uptime', '(pointtouser) (channelname) уже стримит (uptime)');
        $.inidb.set('command', 'followage', '(followage)');
        $.inidb.set('command', 'playtime', '(pointtouser) (channelname) сейчас играет в (game) Время: (playtime)');
        $.inidb.set('command', 'title', '(pointtouser) (titleinfo)');
        $.inidb.set('command', 'game', '(pointtouser) (gameinfo)');
        $.inidb.set('command', 'age', '(age)');

        $.consoleLn('Устанавливаем старые обновления поочередно...');
        ...
        $.consoleLn('Инициализация завершена!');
        sounds = "";
        modules = "";
        versions = "";
        $.changed = false;
    ...
    if (!$.inidb.exists('updates', 'installedv2') || $.inidb.get('updates', 'installedv2') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0...');
        ...
        $.consoleLn('PhantomBot v2.0 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.0.5') || $.inidb.get('updates', 'installedv2.0.5') != 'true') {
        ...
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0.5...');

        $.consoleLn('Отключаем новые стандартные модули...');
        for (i in newDefaultDisabledModules) {
            $.inidb.set('modules', newDefaultDisabledModules[i], 'false');
        }

        $.consoleLn('Удаляем commandCooldown таблицу...');
        $.inidb.RemoveFile('commandCooldown');

        $.consoleLn('PhantomBot v2.0.5 обновление установлено!');
        $.inidb.set('updates', 'installedv2.0.5', 'true');
    }

    if (!$.inidb.exists('updates', 'installedv2.0.6') || $.inidb.get('updates', 'installedv2.0.6') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0.6...');
        ...
        $.consoleLn('PhantomBot v2.0.6 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.0.7') || $.inidb.get('updates', 'installedv2.0.7') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0.7...');
        ...
        */
        ...
        $.consoleLn('PhantomBot v2.0.7 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.0.7.2') || $.inidb.get('updates', 'installedv2.0.7.2') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0.7.2...');
        ...
        $.consoleLn('PhantomBot v2.0.7.2 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.0.8') || $.inidb.get('updates', 'installedv2.0.8') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0.8...');
        ...
        $.consoleLn('PhantomBot v2.0.8 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.0.9') || $.inidb.get('updates', 'installedv2.0.9') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.0.9...');

        $.consoleLn('Удаляем старый кэш Смайликов...');
        $.inidb.del('emotescache', 'emotes');

        $.consoleLn('PhantomBot v2.0.9 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.1.0') || $.inidb.get('updates', 'installedv2.1.0') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.1...');

        $.consoleLn('Группируем !permission к !group...');
        $.inidb.set('aliases', 'group', 'permission');

        $.consoleLn('Группируем !permissionpoints к !grouppoints...');
        $.inidb.set('aliases', 'grouppoints', 'permissionpoints');

        $.consoleLn('Группируем !permissions к !groups...');
        $.inidb.set('aliases', 'groups', 'permissions');

        $.consoleLn('Отключаем новые модули...');
        $.inidb.set('modules', './games/gambling.js', 'false');

        $.consoleLn('Устанавливаем новую задержку постов для Twitter...');
        $.inidb.set('twitter', 'postdelay_update', 180);

        $.consoleLn('PhantomBot v2.1 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.3ss') || $.inidb.get('updates', 'installedv2.3.3ss') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.3...');

        $.consoleLn('Удаляем старый кэш Смайликов.');
        $.inidb.RemoveFile('emotecache');

        $.consoleLn('Обновляем настройки Лотереи...');
        ...
        if ($.inidb.exists('command', 'uptime') && $.inidb.get('command', 'uptime').equalsIgnoreCase('(@sender) (channelname) уже стримит (uptime)')) {
            $.inidb.set('command', 'uptime', '(pointtouser) (channelname) уже стримит (uptime)');
        }

        if ($.inidb.exists('command', 'playtime') && $.inidb.get('command', 'playtime').equalsIgnoreCase('(@sender) (channelname) сейчас играет в (game) Время: (playtime)')) {
            $.inidb.set('command', 'playtime', '(pointtouser) (channelname) сейчас играет в (game) Время: (playtime)');
        }
        ...
        $.consoleLn('PhantomBot v2.3.3 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.5ss') || $.inidb.get('updates', 'installedv2.3.5ss') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.5...');
        ...
        $.consoleLn('Удаляем старые настройки Discord...');
        ...
        $.consoleLn('Перезагружаем BlackList и WhiteList...');
        ...
        $.consoleLn('Обновляем настройки Host...');
        $.inidb.set('settings', 'hostToggle', true);

        $.consoleLn('Отключаем стандартые модули для Discord.');
        modules = [
            './discord/handlers/bitsHandler.js',
            './discord/handlers/followHandler.js',
            './discord/handlers/subscribeHandler.js',
        ...
        $.consoleLn('PhantomBot v2.3.5 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.5.1') || $.inidb.get('updates', 'installedv2.3.5.1') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.5.1...');
        ...
        $.consoleLn('PhantomBot v2.3.5.1 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.5.2') || $.inidb.get('updates', 'installedv2.3.5.2') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.5.2...');

        $.consoleLn('Перезагружаем Цитаты... Пожалуйста не выключайте бота.');
        ...
        $.consoleLn('PhantomBot v2.3.5.2 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.5.3') || $.inidb.get('updates', 'installedv2.3.5.3') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.5.3...');
        ...
        $.consoleLn('PhantomBot v2.3.5.3 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.6') || $.inidb.get('updates', 'installedv2.3.6') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.6...');

        $.consoleLn('Отключаем стандартые модули Discord.');
        ...
        $.consoleLn('PhantomBot v2.3.6 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.6ss') || $.inidb.get('updates', 'installedv2.3.6ss') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.6s...');
        ...
        $.consoleLn('Обновляем Discord cooldowns...');
        for (i in keys) {
        ...
        $.consoleLn('PhantomBot v2.3.6s обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.6b') || $.inidb.get('updates', 'installedv2.3.6b') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.6b...');

        $.consoleLn('Фиксим Верхний регистр имени в таблицах.');
        ...
        $.consoleLn('PhantomBot v2.3.6b обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.7b') || $.inidb.get('updates', 'installedv2.3.7b') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.7...');

        var keys = $.inidb.GetKeyList('blackList', ''),
            timeout = $.getIniDbNumber('chatModerator', 'blacklistTimeoutTime', 600),
            message = $.getIniDbString('chatModerator', 'blacklistMessage', 'Вы получили ТаймАут за Запрещенное слово или выражение.'),
            messageB = $.getIniDbString('chatModerator', 'silentBlacklistMessage', 'Использование запрещенного слова или выражения. (Автоматически ботом: ' + $.botName + ')'),
            obj = {},
            i;
        ...
        $.consoleLn('PhantomBot v2.3.7 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.9') || $.inidb.get('updates', 'installedv2.3.9') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.9...');

        $.consoleLn('Удаляем старые Discord обработчики...');
        $.inidb.del('modules', './handlers/discordHandler.js');
        ...
        $.consoleLn('PhantomBot v2.3.9 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.9.1') || $.inidb.get('updates', 'installedv2.3.9.1') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.9.1...');

        $.consoleLn('Обновляем старые переменные...');

        if ($.inidb.FileExists('discordSettings')) {
            $.inidb.set('discordSettings', 'gameMessage', '(name) только что изменил игру на Twitch!');
            $.inidb.set('discordSettings', 'onlineMessage', '(name) сейчас стримит на Twitch!');
        }

        $.consoleLn('PhantomBot v2.3.9.1 обновление установлено!');
    ...
    if (!$.inidb.exists('updates', 'installedv2.3.9.1b') || $.inidb.get('updates', 'installedv2.3.9.1b') != 'true') {
        $.consoleLn('Запускаем обновление PhantomBot версии 2.3.9.1b...');

        if ($.inidb.FileExists('discordStreamStats')) {
            $.consoleLn('Удаляем старую статистику Discord...');
